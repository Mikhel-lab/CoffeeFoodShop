...
namespace Shop.Web.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(AccountLoginModel login)
        {
            ...
            if (user != null)
            {
                var result = await _signInManager.PasswordSignInAsync(user, login.Password,isPersistent: false, false);
                if (result.Succeeded)
                {
        ...
        public IActionResult HashPassword()
        {
            ...
            {
                UserName = "tchiboadmin@mail.com",
                Email = "tchiboadmin@mail.com",
            };
            PasswordHasher<ApplicationUser> hasher = new PasswordHasher<ApplicationUser>();
           // appUser.PasswordHash = hasher.HashPassword(appUser, "Icui4cubis@");
            var newPassword = appUser.PasswordHash = hasher.HashPassword(appUser, "Icui4cubis@");
            ViewBag.Data = newPassword;
            return View();
        }
    }
}